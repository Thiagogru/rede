    ...
    while True:
        try:
            ...
            if message.startswith("FILE:"):
                ...
                clear_line()
                sys.stdout.write(f"Mensagem recebida do servidor: {message}\n")
                sys.stdout.write("Digite a mensagem para enviar ao servidor: ")
                sys.stdout.flush()
...
def send_messages(send_port, server_ip):
    ...
    while True:
        try:
            ...
            if message.startswith("FILE:"):
                ...
                send_file(file_path, server_ip, send_port)
            else:
                client_socket.sendto(message.encode(), (server_ip, send_port))
...
def send_file(file_path, server_ip, send_port):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    with open(file_path, "rb") as f:
        ...
        client_socket.sendto(
            f"FILE:{file_name}".encode(), (server_ip, send_port))
        while True:
            chunk = f.read(1024)
            if not chunk:
                break
            client_socket.sendto(chunk, (server_ip, send_port))
        client_socket.sendto(b"EOF", (server_ip, send_port))
...
if __name__ == "__main__":
    ...
    send_port = int(
        input("Digite a porta para enviar mensagens ao servidor: "))
    receive_port = int(
        input("Digite a porta para receber mensagens do servidor: "))
    ...
    send_thread = threading.Thread(
        target=send_messages, args=(send_port, server_ip))
    send_thread.daemon = True
    send_thread.start()

    # Manter o cliente em execução
    receive_thread.join()
    send_thread.join()
